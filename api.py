...
class API:
    def __init__(self):
        ...
        self.allData = {}
        self.baseAPI = "https://api.covid19api.com"
    ...
    def summary_by_country(self, searched_country) -> dict:

        searched_country = searched_country.upper()

        url = self.baseAPI + "/summary"
        r = requests.get(url)
        ...
        if r.status_code == 200:
            ...
            for country in data["Countries"]:
                if country["CountryCode"] == searched_country:
                    return country

            return {"error": "country code not recognised"}

        return {"error": r.status_code}

    def summary_worldwide(self) -> dict:
        url = self.baseAPI + "/world/total"
